...
public class OpenCloseUIView extends Composite implements OpenCloseView {

	private static OpenCloseUIViewUiBinder uiBinder = GWT
			.create(OpenCloseUIViewUiBinder.class);

	interface OpenCloseUIViewUiBinder extends UiBinder<Widget, OpenCloseUIView> {
	}
	...
	@UiField
	InputElement radioClose;
	
	@UiField
	InputElement radioOpen;
	...
	public OpenCloseUIView() {
		...
		groupRoomUI.setStyleName("rooms-check-list");
  	    ...
  	    this.radioClose.setAttribute("id", "radio2");
  	    this.radioOpen.setAttribute("id", "radio1");
  	    this.radioOpen.setChecked(true);
  	    otaCharge = false;
  	    roomCharge = false;
  	    this.popupInfo.setAttribute("id", "update-openclose-popup");
	}
	...
	public void chargeOtas(List<OtaDTO> otas){
		...
		for (OtaDTO otaDTO : otas) {
			otaWidget = new OtaWidgetUIView(otaDTO,ActionEnum.OPEN_CLOSE);
	...
	public void chargeRooms(List<GroupRoomDTO> rooms) {
		...
		for (GroupRoomDTO group : rooms) {
			widget = new GroupRoomWidgetUIView(group,ActionEnum.OPEN_CLOSE);
	...
	@UiHandler("submit")
	public void onClickSubmitOpenClose(ClickEvent event){
		String dates = this.selectedDates.getValue();
		this.presenter.doChangeOpenClose(dates,this.radioOpen.isChecked());
	}
