...
public class RateUpdateUIView extends Composite implements RateUpdateView {

	private static RateUIViewUiBinder uiBinder = GWT
			.create(RateUIViewUiBinder.class);

	interface RateUIViewUiBinder extends UiBinder<Widget, RateUpdateUIView> {
	}
	...
	public RateUpdateUIView() {
		...
		groupRoomUI.setStyleName("rooms-check-list");
  	    ...
  	    otaCharge = false;
  	    roomCharge = false;
  	    this.popupInfo.setAttribute("id", "update-rate-popup");
	}
	...
	public void chargeOtas(List<OtaDTO> otas){
		...
		for (OtaDTO otaDTO : otas) {
			otaWidget = new OtaWidgetUIView(otaDTO,ActionEnum.RATE_UPDATE);
	...
	public void chargeRooms(List<GroupRoomDTO> rooms) {
		...
		for (GroupRoomDTO group : rooms) {
			widget = new GroupRoomWidgetUIView(group,ActionEnum.RATE_UPDATE);
	...
	@UiHandler("submit")
	public void onClickSubmitRate(ClickEvent event){
		String dates = this.selectedDates.getValue();
		String value = this.price.getValue();
		this.presenter.doChangeRate(dates,new Double(value));
	}
